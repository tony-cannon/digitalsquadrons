...
class BuddyDev_Username_Availability_Checker {
	...
	public function load_js() {
		...
		if ( $this->should_load_asset() ) {
			//wp_enqueue_script( 'ds-form-validation', get_stylesheet_directory_uri() . '/assets/js/jquery.validate.min.js', array( 'jquery' ) );
			wp_enqueue_script( 'username-availability-checker-js', get_stylesheet_directory_uri() . '/assets/js/username-availability-checker.js', array( 'jquery' ) );

			$data = array(
				'selectors' => apply_filters( 'buddydev_uachecker_selectors', 'input#signup_username, form#createuser input#user_login, #registerform input#user_login, .lwa-register input#user_login, #signup-form input#field_3' ),
			);
	...
	public function load_css() {
		...
		if ( $this->should_load_asset() ) {
			wp_enqueue_style( 'username-availability-checker-css', get_stylesheet_directory_uri() . '/assets/css/username-availability-checker.css' );
		}
	}
